...
const Main = ({items,loading}) => {
    ...
    useEffect(()=>{  
        ...
        tl.fromTo(elements, {y: '+=100'},{duration: 1, y:'-=100', autoAlpha:'1'})
    },[loading])
    ...
    const closeModal = () => {setIsOpen(false)};
     ...
     const Bookcase = Books.map((item,i) => (
        ...
        <Book key={item.id} >
            ...
            <img className = {item.id}src={item.volumeInfo.imageLinks === undefined ? "https://www.gollancz.co.uk/wp-content/uploads/2018/07/missingbook.png":item.volumeInfo.imageLinks.smallThumbnail} alt={item.volumeInfo.title}/>
            <Button onClick ={(e) => openModal(e)} className={ i+1}>Details</Button>
    ...
    return(
        <main>
            <BookModal opened={modalIsOpen} closeModal = {closeModal} info = {info} saleLink = {saleLink}/>
